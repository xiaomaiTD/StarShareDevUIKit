...
const CGSize UIBubbleViewAutomaticalMaximumItemSize = {-1, -1};
...
- (void)didInitialized {
  ...
  self.maximumItemSize = UIBubbleViewAutomaticalMaximumItemSize;
}
...
- (CGSize)layoutSubviewsWithSize:(CGSize)size shouldLayout:(BOOL)shouldLayout {
  ...
  CGSize maximumItemSize = CGSizeEqualToSize(self.maximumItemSize, UIBubbleViewAutomaticalMaximumItemSize) ? CGSizeMake(size.width - UIEdgeInsetsGetHorizontalValue(self.padding), size.height - UIEdgeInsetsGetVerticalValue(self.padding)) : self.maximumItemSize;
