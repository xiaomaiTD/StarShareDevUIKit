 ...
 *
 * 拓展性：`UIToastBackgroundView`和`UIToastContentView`是提供的默认的view，这两个view都可以通过appearance来修改样式，如果这两个view满足不了需求，那么也可以通过新建自定义的view来代替这两个view。另外也提供了默认的toastAnimator来实现ToastView的显示和隐藏动画，如果需要重新定义一套动画，可以继承`UIToastAnimator`并且实现`UIToastViewAnimatorDelegate`中的协议就可以自定义自己的一套动画。
 *
