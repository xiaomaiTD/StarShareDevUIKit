...
// generate icon images needed by SSUIZoomImageView
// 用于生成 SSUIZoomImageView 所需的一些简单的图标图片
@interface SSUIZoomImageViewImageGenerator : NSObject
...
@interface SSUIZoomImageVideoPlayerView : UIView

@end

static NSUInteger const kTagForCenteredPlayButton = 1;

@interface SSUIZoomImageView () <UIGestureRecognizerDelegate>

@property(nonatomic, strong) UIScrollView *scrollView;

// video play
@property(nonatomic, strong) SSUIZoomImageVideoPlayerView *videoPlayerView;
...
@implementation SSUIZoomImageView
...
- (instancetype)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    SSUIZoomImageView *appearance = [SSUIZoomImageView appearance];
    ...
    _emptyView = [[UIEmptyView alloc] init];
...
- (void)setZoomScale:(CGFloat)zoomScale animated:(BOOL)animated {
  if (animated) {
    [UIView animateWithDuration:.25 delay:0.0 options:UIViewAnimationOptionsCurveOut animations:^{
...
- (void)addPlayerTimeObserver {
  ...
  double interval = .1f;
  __weak SSUIZoomImageView *weakSelf = self;
...
- (SSUIZoomImageViewVideoToolbar *)videoToolbar {
  [self initVideoToolbarIfNeeded];
  return _videoToolbar;
}

- (SSUIButton *)videoCenteredPlayButton {
...
- (void)initVideoPlayerLayerIfNeeded {
  ...
  self.videoPlayerView = [[SSUIZoomImageVideoPlayerView alloc] init];
...
- (void)initVideoToolbarIfNeeded {
  ...
  _videoToolbar = ({
    SSUIZoomImageViewVideoToolbar * b = [[SSUIZoomImageViewVideoToolbar alloc] init];
...
- (void)initVideoCenteredPlayButtonIfNeeded {
  ...
  _videoCenteredPlayButton = ({
    SSUIButton *b = [[SSUIButton alloc] init];
    b.outsideEdge = UIEdgeInsetsMake(-60, -60, -60, -60);
...
@interface SSUIZoomImageView (UIAppearance)

@end

@implementation SSUIZoomImageView (UIAppearance)
...
+ (void)setDefaultAppearance {
  SSUIZoomImageView *appearance = [SSUIZoomImageView appearance];
  appearance.videoCenteredPlayButtonImage = [SSUIZoomImageViewImageGenerator largePlayImage];
}

@end

@implementation SSUIZoomImageVideoPlayerView
...
@implementation SSUIZoomImageViewImageGenerator
...
@implementation SSUIZoomImageViewVideoToolbar

- (instancetype)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    SSUIZoomImageViewVideoToolbar *appearance = [SSUIZoomImageViewVideoToolbar appearance];
    ...
    _playButton = [[SSUIButton alloc] init];
    ...
    _pauseButton = [[SSUIButton alloc] init];
    self.pauseButton.outsideEdge = UIEdgeInsetsMake(-10, -10, -10, -10);
    [self.pauseButton setImage:self.pauseButtonImage forState:UIControlStateNormal];
    [self addSubview:self.pauseButton];
    
    _slider = [[SSUISlider alloc] init];
    ...
    _sliderRightLabel = [[UILabel alloc]  initWithFont:UIFontMake(12) textColor:UIColorWhite];
...
@interface SSUIZoomImageViewVideoToolbar (UIAppearance)

@end

@implementation SSUIZoomImageViewVideoToolbar (UIAppearance)
...
+ (void)setDefaultAppearance {
  ...
  appearance.playButtonImage = [SSUIZoomImageViewImageGenerator smallPlayImage];
  appearance.pauseButtonImage = [SSUIZoomImageViewImageGenerator pauseImage];
}
