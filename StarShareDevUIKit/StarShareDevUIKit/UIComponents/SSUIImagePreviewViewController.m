//
//  SSUIImagePreviewViewController.m
//  StarShareDevUIKit
//
...
@implementation SSUIImagePreviewViewController (UIAppearance)
...
static SSUIImagePreviewViewController *imagePreviewViewControllerAppearance;
+ (instancetype)appearance {
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    if (!imagePreviewViewControllerAppearance) {
      imagePreviewViewControllerAppearance = [[SSUIImagePreviewViewController alloc] init];
...
@interface SSUIImagePreviewViewController ()
...
@implementation SSUIImagePreviewViewController
...
- (void)didInitialized {
  ...
  if (imagePreviewViewControllerAppearance) {
    self.backgroundColor = [SSUIImagePreviewViewController appearance].backgroundColor;
  }
}

- (SSUIImagePreviewView *)imagePreviewView {
...
- (void)initSubviews {
  [super initSubviews];
  _imagePreviewView = [[SSUIImagePreviewView alloc] initWithFrame:self.view.bounds];
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  // 配合 SSUIImagePreviewViewController (UIWindow) 使用的
  if (self.previewWindow) {
    ...
    SSUIZoomImageView *zoomImageView = [self.imagePreviewView zoomImageViewAtIndex:self.imagePreviewView.currentImageIndex];
    if (!zoomImageView) {
...
@implementation SSUIImagePreviewViewController (UIWindow)
...
- (void)initPreviewWindowIfNeeded {
  if (!self.previewWindow) {
    ...
    self.previewWindow.windowLevel = UIWindowLevelUIImagePreviewView;
