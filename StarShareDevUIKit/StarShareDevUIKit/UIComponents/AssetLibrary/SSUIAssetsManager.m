...
void SSUIImageWriteToSavedPhotosAlbumWithAlbumAssetsGroup(UIImage *image, SSUIAssetsGroup *albumAssetsGroup, SSUIWriteAssetCompletionBlock completionBlock) {
  [[SSUIAssetsManager sharedInstance] saveImageWithImageRef:image.CGImage albumAssetsGroup:albumAssetsGroup orientation:image.imageOrientation completionBlock:completionBlock];
}

void SSUISaveImageAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *imagePath, SSUIAssetsGroup *albumAssetsGroup, SSUIWriteAssetCompletionBlock completionBlock) {
  [[SSUIAssetsManager sharedInstance] saveImageWithImagePathURL:[NSURL fileURLWithPath:imagePath] albumAssetsGroup:albumAssetsGroup completionBlock:completionBlock];
}

void SSUISaveVideoAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *videoPath, SSUIAssetsGroup *albumAssetsGroup, SSUIWriteAssetCompletionBlock completionBlock) {
  [[SSUIAssetsManager sharedInstance] saveVideoWithVideoPathURL:[NSURL fileURLWithPath:videoPath] albumAssetsGroup:albumAssetsGroup completionBlock:completionBlock];
}

@implementation SSUIAssetsManager{
  PHCachingImageManager *_cachingImageManager;
}

+ (SSUIAssetsManager *)sharedInstance {
  static dispatch_once_t onceToken;
  static SSUIAssetsManager *instance = nil;
...
+ (SSUIAssetAuthorizationStatus)authorizationStatus {
  SSUIAssetAuthorizationStatus status;
  PHAuthorizationStatus authorizationStatus = [PHPhotoLibrary authorizationStatus];
  if (authorizationStatus == PHAuthorizationStatusRestricted || authorizationStatus == PHAuthorizationStatusDenied) {
    status = SSUIAssetAuthorizationStatusNotAuthorized;
  } else if (authorizationStatus == PHAuthorizationStatusNotDetermined) {
    status = SSUIAssetAuthorizationStatusNotDetermined;
  } else {
    status = SSUIAssetAuthorizationStatusAuthorized;
  }
  return status;
}
...
- (void)enumerateAllAlbumsWithAlbumContentType:(SSUIAlbumContentType)contentType showEmptyAlbum:(BOOL)showEmptyAlbum showSmartAlbumIfSupported:(BOOL)showSmartAlbumIfSupported usingBlock:(void (^)(SSUIAssetsGroup *resultAssetsGroup))enumerationBlock {
  ...
  // 创建一个 PHFetchOptions，用于 SSUIAssetsGroup 对资源的排序以及对内容类型进行控制
  PHFetchOptions *fetchOptions = [PHPhotoLibrary createFetchOptionsWithAlbumContentType:contentType];
  
  // 遍历结果，生成对应的 SSUIAssetsGroup，并调用 enumerationBlock
  for (NSUInteger i = 0; i < [tempAlbumsArray count]; i++) {
    PHAssetCollection *assetCollection = [tempAlbumsArray objectAtIndex:i];
    SSUIAssetsGroup *assetsGroup = [[SSUIAssetsGroup alloc] initWithCollection:assetCollection fetchAssetsOptions:fetchOptions];
    if (enumerationBlock) {
...
- (void)saveImageWithImageRef:(CGImageRef)imageRef albumAssetsGroup:(SSUIAssetsGroup *)albumAssetsGroup orientation:(UIImageOrientation)orientation completionBlock:(SSUIWriteAssetCompletionBlock)completionBlock {
  PHAssetCollection *albumPhAssetCollection = albumAssetsGroup.assetCollection;
                                  ...
                                  albumAssetCollection:albumPhAssetCollection
                                     ...
                                     completionHandler:^(BOOL success, NSDate *creationDate, NSError *error) {
                                       if (success) {
                                         ...
                                         completionBlock(ssui_asset, error);
                                       } else {
                                         SSUIKitLog(@"Get PHAsset of image error: %@", error);
                                  ...
                                  albumAssetCollection:albumPhAssetCollection
                                     completionHandler:^(BOOL success, NSDate *creationDate, NSError *error) {
                                       if (success) {
                                         ...
                                         SSUIKitLog(@"Get PHAsset of video Error: %@", error);
                                         completionBlock(nil, error);
                                       }
                                     }];
}
...
+ (NSArray *)fetchAllAlbumsWithAlbumContentType:(SSUIAlbumContentType)contentType showEmptyAlbum:(BOOL)showEmptyAlbum showSmartAlbum:(BOOL)showSmartAlbum {
  ...
  // 创建一个 PHFetchOptions，用于创建 SSUIAssetsGroup 对资源的排序和类型进行控制
  PHFetchOptions *fetchOptions = [PHPhotoLibrary createFetchOptionsWithAlbumContentType:contentType];
...
- (void)addImageToAlbum:(UIImage *)image imagePathURL:(NSURL *)imagePathURL albumAssetCollection:(PHAssetCollection *)albumAssetCollection completionHandler:(void(^)(BOOL success, NSDate *creationDate, NSError *error))completionHandler {
  ...
  [self performChanges:^{
    ...
    if (image) {
      ...
      SSUIKitLog(@"Creating asset with empty data");
      return;
    }
    ...
    if (!success) {
      SSUIKitLog(@"Creating asset of image error : %@", error);
    }
...
- (void)addVideoToAlbum:(NSURL *)videoPathURL albumAssetCollection:(PHAssetCollection *)albumAssetCollection completionHandler:(void(^)(BOOL success, NSDate *creationDate, NSError *error))completionHandler {
  ...
  [self performChanges:^{
    ...
    if (!success) {
      SSUIKitLog(@"Creating asset of video error: %@", error);
    }
