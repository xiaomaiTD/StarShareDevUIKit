...
- (NSString *)ss_description {
  return [NSString stringWithFormat:@"%@, scale = %@", [self ss_description], @(self.scale)];
}
...
- (UIImage *)grayImage {
  ...
  if (self.opaque) {
    ...
    // 用 CGBitmapContextCreateImage 方式创建出来的图片，CGImageAlphaInfo 总是为 CGImageAlphaInfoNone，导致 opaque 与原图不一致，所以这里再做多一步
...
- (UIImage *)imageWithBlendColor:(UIColor *)blendColor {
  UIImage *coloredImage = [self imageWithTintColor:blendColor];
...
- (UIImage *)imageWithScaleToSize:(CGSize)size {
  return [self imageWithScaleToSize:size contentMode:UIViewContentModeScaleAspectFit];
}
...
- (UIImage *)imageWithScaleToSize:(CGSize)size contentMode:(UIViewContentMode)contentMode scale:(CGFloat)scale {
  ...
  UIGraphicsBeginImageContextWithOptions(drawingRect.size, self.opaque, scale);
  CGContextRef context = UIGraphicsGetCurrentContext();
  CGContextInspectContext(context);
...
- (UIImage *)imageWithBorderColor:(UIColor *)borderColor path:(UIBezierPath *)path {
  ...
  UIGraphicsBeginImageContextWithOptions(oldImage.size, self.opaque, oldImage.scale);
  CGContextRef context = UIGraphicsGetCurrentContext();
  CGContextInspectContext(context);
...
- (UIImage *)imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth borderPosition:(UIImageBorderPosition)borderPosition {
  if (borderPosition == UIImageBorderPositionAll) {
    ...
    if ((UIImageBorderPositionBottom & borderPosition) == UIImageBorderPositionBottom) {
...
+ (UIImage *)imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth borderPosition:(UIImageBorderPosition)borderPosition {
  CGContextInspectSize(size);
  if (borderPosition == UIImageBorderPositionAll) {
