...
- (void)handleTextChanged:(id)sender {
  ...
  SSUITextView *textView = nil;
  
  if ([sender isKindOfClass:[NSNotification class]]) {
    id object = ((NSNotification *)sender).object;
    if (object == self) {
      textView = (SSUITextView *)object;
    }
  } else if ([sender isKindOfClass:[SSUITextView class]]) {
    textView = (SSUITextView *)sender;
  }
  ...
  if (textView) {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
      ...
      [self scrollCaretVisibleAnimated:NO];
    });
  }
}
...
- (void)textViewDidChange:(SSUITextView *)textView {
...
- (void)setDelegate:(id<SSUITextViewDelegate>)delegate {
