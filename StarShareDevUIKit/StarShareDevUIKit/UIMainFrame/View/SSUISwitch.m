...
@interface SSUISwitch () <UIGestureRecognizerDelegate> {
...
@implementation SSUISwitch

- (id)initWithFrame:(CGRect)frame
{
  if (CGRectEqualToRect(frame, CGRectZero)) {
    frame = CGRectMake(0, 0, 49, 31);
  }
...
- (void)configure {
  ...
  SSUISwitch *appearance = [SSUISwitch appearance];
  _tintColor = appearance.tintColor;
  _onTintColor = appearance.onTintColor;
  _offTintColor = appearance.offTintColor;
  _thumbTintColor = appearance.thumbTintColor;
  
  [self setBackgroundColor:[UIColor clearColor]];
  
  _backLayer.fillColor = [_tintColor CGColor];
  _fillLayer.fillColor = [_onTintColor CGColor];
...
{
  if (_backLayer != nil && ![[_backLayer valueForKey:@"isOn"] boolValue]) {
    _backLayer.fillColor = [_tintColor CGColor];
  }
}
...
- (void)setOffTintColor:(UIColor *)offTintColor
{
  ...
  [self setBacklayeroffTintColorIfNeeded];
}

- (void)setBacklayeroffTintColorIfNeeded
{
  if (_fillLayer != nil) {
    _fillLayer.fillColor = [_offTintColor CGColor];
  }
}

- (void)setThumbTintColor:(UIColor *)thumbTintColor {
  _thumbTintColor = thumbTintColor;
  [self setThumblayerTintColorIfNeeded];
}

- (void)setThumblayerTintColorIfNeeded
{
  if (_thumbLayer != nil) {
    _fillLayer.fillColor = [_offTintColor CGColor];
  }
...
@interface SSUISwitch (UIAppearance)

@end

@implementation SSUISwitch (UIAppearance)
...
+ (void)setDefaultAppearance {
  SSUISwitch *appearance = [SSUISwitch appearance];
  appearance.tintColor = [UIColor colorWithRed:0.90f green:0.90f blue:0.90f alpha:1.00f];
  appearance.onTintColor = [UIColor colorWithRed:0.27f green:0.85f blue:0.37f alpha:1.00f];
  appearance.offTintColor = [UIColor whiteColor];
  appearance.thumbTintColor = [UIColor whiteColor];
}
